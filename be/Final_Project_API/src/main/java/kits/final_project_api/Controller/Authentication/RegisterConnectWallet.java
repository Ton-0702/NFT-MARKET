...
public class RegisterConnectWallet {
    ...
    @PostMapping
    public ResponseEntity<RegisterResponseDto> registerCreateAccount(
            @Valid @RequestParam(value = "address_wallet") String address_wallet,
        ...
        // System.out.println("formData.get(address_wallet).toString():
        // "+formData.get("address_wallet").toString());
        ...
        if (foundAccountAddressWallet == null) {
            try {
                ...
                if (foundAccountEmail == null && foundAccount == null) {
                    ...
                    account_create.setPrice(price);
                    account_create.setAvatar(image);
                    account_create.setBackground(background);
                    account_create.setBiography(bio);
                    // Tạo token ngẫu nhiên qua class UtilsService và hàm getRandomHexString trong
                    // class đó, sau đó gắn vào token entity account
                    ...
                    return ResponseEntity.status(HttpStatus.OK).body(new RegisterResponseDto("Created Successfully",
                            account_create.getToken(), "", "Created_Succesfully"));
                } else {
                    if (foundAccount != null) {
                        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                                .body(new RegisterResponseDto("Username Exist", account_create, "", "Username_Exist"));
                    } else {
                        if (foundAccountEmail != null) {
                            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                                    .body(new RegisterResponseDto("Email Exist", account_create, "", "Email_Exist"));
                        }
                    }
                }
            } catch (Exception e) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new RegisterResponseDto("", "", e.getMessage(),
                                "UNKNOWN_ERROR"));
            }
        }
        return ResponseEntity.status(HttpStatus.OK)
                .body(new RegisterResponseDto("Address Wallet already exists", "", "", "Address_Wallet_EXIST"));
    }
}
